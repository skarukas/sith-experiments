...
class PeriodicRelativeEncoding(nn.Module):
    def __init__(self, period, dim=-1, real=False):
        ...
        Note: (I think) the complex representation satisfies 
          ...
          the DNN would have to learn to disambiguate the output.
        """
        super().__init__()
        self.dim = dim
        self.real = real
...
if __name__ == "__main__":
    ...
    for real in (True, False):
        for dim in range(len(x.shape)):
            ...
            # add random multiple of period to each
            shape = [1] * len(x.shape)
